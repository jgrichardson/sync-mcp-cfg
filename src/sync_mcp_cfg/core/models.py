...
class MCPServer(BaseModel):
    ...
    server_type: MCPServerType = Field(
        default=MCPServerType.STDIO, description="Server transport type"
    )
...
class ClientConfig(BaseModel):
    ...
    is_available: bool = Field(
        default=False, description="Whether the client is installed/available"
    )
    servers: List[MCPServer] = Field(
        default_factory=list, description="Servers configured for this client"
    )
...
class SyncConfig(BaseModel):
    ...
    servers: List[str] = Field(
        default_factory=list, description="Specific servers to sync (empty = all)"
    )
    overwrite: bool = Field(
        default=False, description="Whether to overwrite existing servers"
    )
    backup: bool = Field(
        default=True, description="Whether to create backup before sync"
    )
    ...
    def __str__(self) -> str:
        ...
        return (
            f"Sync {server_count} servers from {self.source_client.value} to {targets}"
        )
...
class AppConfig(BaseModel):
    ...
    auto_backup: bool = Field(
        default=True, description="Automatically backup before changes"
    )
    backup_retention_days: int = Field(default=30, description="Days to keep backups")
    default_sync_target: Optional[List[ClientType]] = Field(
        None, description="Default sync targets"
    )
    validate_servers: bool = Field(
        default=True, description="Validate server configurations"
    )
