...
@click.command()
@click.argument('name')
@click.option('--clients', '-c', multiple=True,
...
def remove_command(
    ...
    if not servers_to_remove:
        console.print(f"[yellow]Server '{name}' not found in any of the specified clients.[/yellow]")
        return
    ...
    for client_type, server in servers_to_remove:
        table.add_row(
            client_type.value,
            f"{server.command} {' '.join(server.args)}" if server.args else server.command,
    ...
    for client_type, server in servers_to_remove:
        try:
            ...
            if handler.remove_server(name):
                console.print(f"[green]✓ Removed '{name}' from {client_type.value}[/green]")
                success_count += 1
            else:
                console.print(f"[yellow]! Server '{name}' not found in {client_type.value}[/yellow]")
                
        except ClientHandlerError as e:
            console.print(f"[red]✗ Failed to remove from {client_type.value}: {e}[/red]")
        except Exception as e:
            console.print(f"[red]✗ Unexpected error with {client_type.value}: {e}[/red]")
...
def remove_interactive(ctx: click.Context, client: Optional[str], force: bool) -> None:
    ...
    # Confirm removal
    if not force:
        if not Confirm.ask(f"Remove server '{server_to_remove.name}' from {client_type.value}?"):
            console.print("[yellow]Operation cancelled.[/yellow]")
            return
    ...
    try:
        if handler.remove_server(server_to_remove.name):
            console.print(f"[green]✓ Removed '{server_to_remove.name}' from {client_type.value}[/green]")
        else:
            console.print(f"[yellow]Server '{server_to_remove.name}' not found.[/yellow]")
    except Exception as e:
