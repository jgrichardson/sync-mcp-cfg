...
@click.option('--type', 'server_type', type=click.Choice(['stdio', 'sse', 'http']), 
...
def add_command(
    ...
    if clients:
        ...
        while True:
            try:
                selection = Prompt.ask("Enter client numbers (comma-separated) or 'all'")
                if selection.lower() == 'all':
                    ...
                    if all(1 <= i <= len(available_clients) for i in indices):
                        target_clients = [available_clients[i-1].client_type for i in indices]
                        break
                    else:
    ...
    for client_type in target_clients:
        try:
            ...
            if existing_server:
                if not Confirm.ask(f"Server '{name}' already exists in {client_type.value}. Overwrite?"):
