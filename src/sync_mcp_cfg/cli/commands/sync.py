...
@click.option(
    ...
    required=True,
    help='Source client to sync from',
)
@click.option(
    '--to',
    'target_clients',
...
@click.option(
    '--overwrite', is_flag=True, help='Overwrite existing servers without confirmation'
)
...
def sync_command(
    ...
    if servers:
        ...
        if missing_servers:
            console.print(
                f"[red]Servers not found in {source_client}: {', '.join(missing_servers)}[/red]"
            )
            return
        ...
        for client_name in target_clients:
            try:
                ...
                if client_type == source_client_type:
                    console.print(
                        f"[yellow]Skipping source client {client_name}[/yellow]"
                    )
                    continue
        ...
        try:
            ...
            if backup:
                try:
                    ...
                    console.print(
                        f"[green]✓ Created backup for {target_client_type.value}: {backup_path}[/green]"
                    )
                except Exception as e:
                    console.print(
                        f"[yellow]⚠ Failed to create backup for {target_client_type.value}: {e}[/yellow]"
                    )
            ...
            for server in source_servers:
                ...
                if server.name in existing_names and not overwrite:
                    if not Confirm.ask(
                        f"Server '{server.name}' exists in {target_client_type.value}. Overwrite?"
                    ):
                        console.print(
                            f"[yellow]- Skipped '{server.name}' in {target_client_type.value}[/yellow]"
                        )
                        continue
                ...
                try:
                    ...
                    console.print(
                        f"[green]✓ Synced '{server.name}' to {target_client_type.value}[/green]"
                    )
                    client_success += 1
                except Exception as e:
                    console.print(
                        f"[red]✗ Failed to sync '{server.name}' to {target_client_type.value}: {e}[/red]"
                    )

            if client_success > 0:
                success_count += client_success
                console.print(
                    f"[bold green]✓ Successfully synced {client_success} servers to {target_client_type.value}[/bold green]"
                )

        except ClientNotFoundError:
            console.print(
                f"[red]✗ Target client {target_client_type.value} not available[/red]"
            )
        except Exception as e:
            console.print(
                f"[red]✗ Error syncing to {target_client_type.value}: {e}[/red]"
            )
    ...
    if success_count == total_operations:
        ...
        console.print(
            "[yellow]Some operations failed. Check output above for details.[/yellow]"
        )
    else:
...
def sync_interactive(ctx: click.Context) -> None:
    ...
    try:
        ...
        if not source_servers:
            console.print(
                f"[yellow]No servers found in {source_client.client_type.value}[/yellow]"
            )
            return
    except Exception as e:
    ...
    console.print("\nSelect servers to sync:")
    console.print(
        "Enter 'all' for all servers, or comma-separated numbers (e.g., 1,3,5)"
    )
    ...
    target_options = [
        c for c in available_clients if c.client_type != source_client.client_type
    ]
    ...
    overwrite = Confirm.ask(
        "Overwrite existing servers without prompting?", default=False
    )
